...
- UNITE-DB often stores the **pre-evolution** Ability as `skills[].name` and later forms as `passive2_name` / `passive3_name`. `PLAYABLE_PASSIVE_SLUGS` promotes the last name when the license is not on `FORM_PASSIVE_STAGES`. Stills that show each staged Ability go in `FORM_PASSIVE_STAGES` as `(ability, form-name chip)` rows (Sylveon Eevee/Sylveon, Tyranitar Larvitar/Pupitar/Tyranitar, Urshifu Kubfu/Urshifu, …).
- **Mega licenses** are a second path: Pre-Mega + Mega rows via `mega_passive_slots` / `extraPassives`. Three-stage Mega stills can prepend a Magikarp (etc.) chip through `MEGA_PRE_EVO_STAGE` (Mega Gyarados **Swift Swim**). Do not put `mega-charizard-x` (and similar) on `PLAYABLE_PASSIVE_SLUGS` or `FORM_PASSIVE_STAGES` — that path *replaces* one Ability.
...
| Copy fan-site (Serebii / Bulbapedia / Unite-DB wiki / Game8) prose as Basic | Not a live source. There is no scraper or ingest. Do not restore one. |
...
1. **Tooltip stills beat stand-in archive text.** Capture habit: Practice → Ability/move tooltip → PNG into `screenshot-references/move-descriptions/<id>/`. Agent can read PNG/JPEG with the Read tool and check spelling against the archive. Video is not required.
...
| Charizard | `9e0fcf2` | 2026-09-10 operator in-game transcript (Flame Burst and Flamethrower start with **Attacks with**) |
...
Playable Ability + Basic (not Advanced), including pre-evolution stills as form-pair chips: Aegislash No Guard (Honedge) + Stance Change; Ceruledge Flame Body (Charcadet) + Weak Armor; Dragonite Marvel Scale (Dragonair) + Multiscale; Espeon Anticipation (Eevee) + Magic Bounce; Glaceon Run Away (Eevee) + Snow Cloak; Gyarados Rattled (Magikarp) + Moxie; Leafeon Run Away (Eevee) + Chlorophyll; Raichu Static (Pikachu) + Surge Surfer; Solgaleo Unaware (Cosmog) + Sturdy (Cosmoem) + Full Metal Body; Tsareena Oblivious (Bounsweet) + Queenly Majesty; Tyranitar Guts (Larvitar) + Shed Skin (Pupitar) + Sand Stream; Umbreon Anticipation (Eevee) + Inner Focus; Urshifu Inner Focus (Kubfu) + Unseen Fist; Vaporeon Run Away (Eevee) + Water Absorb.

Mega-stage Basic: Mega Charizard X Tough Claws; Mega Charizard Y Drought; Mega Lucario Adaptability; Mega Gyarados Swift Swim (Magikarp) + Intimidate (Pre-Mega) + Mold Breaker (Mega).
